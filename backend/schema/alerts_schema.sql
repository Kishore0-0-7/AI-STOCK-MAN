-- ============================================================================
-- AI STOCK MANAGEMENT - LOW STOCK ALERTS SYSTEM DATABASE SCHEMA
-- ============================================================================
-- Complete database setup for Low Stock Alerts with Purchase Order management
-- Includes all tables, triggers, indexes, sample data, and utility queries
-- ============================================================================

-- ============================================================================
-- 1. CREATE DATABASE (Optional - if needed)
-- ============================================================================
-- CREATE DATABASE IF NOT EXISTS ai_stock_management;
-- USE ai_stock_management;

-- ============================================================================
-- 2. TABLE MODIFICATIONS & CREATION
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS suppliers (
  ...
  payment_terms INT DEFAULT 30, -- Payment terms in days
...
-- Add columns to existing products table for alert management
ALTER TABLE products 
ADD COLUMN IF NOT EXISTS low_stock_threshold INT DEFAULT 10,
ADD COLUMN IF NOT EXISTS low_stock_alert_time TIMESTAMP NULL,
ADD COLUMN IF NOT EXISTS alert_status ENUM('active', 'ignored', 'resolved') DEFAULT 'active',
...
ADD COLUMN IF NOT EXISTS alert_priority ENUM('low', 'medium', 'high') DEFAULT 'medium',
...
-- Create alert_history table for tracking all alert activities
CREATE TABLE IF NOT EXISTS alert_history (
  ...
  notes TEXT,
  metadata JSON, -- Store additional data like PO numbers, ignore reasons, etc.
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (product_id) REFERENCES products(id) ON DELETE CASCADE,
...
-- Create po_line_items table for multiple products per PO
...
-- Trigger to automatically set low_stock_alert_time when stock goes below threshold
DELIMITER $$
...
FOR EACH ROW
BEGIN
  -- Check if stock is dropping below threshold
  ...
  -- Reset alert if stock goes back above threshold
  IF NEW.current_stock > NEW.low_stock_threshold AND 
...
-- Trigger to generate PO number automatically
...
BEGIN
  IF NEW.po_number IS NULL OR NEW.po_number = '' THEN
    SET NEW.po_number = CONCAT('PO', YEAR(NOW()), LPAD(MONTH(NOW()), 2, '0'), LPAD(NEW.id, 4, '0'));
  END IF;
END$$

-- Trigger to update total amount when line items change
...
-- Insert sample products with comprehensive alert data
...
-- Trigger the alert system by updating stock levels to create alerts
UPDATE products SET low_stock_alert_time = NOW() WHERE current_stock <= low_stock_threshold;

-- Insert sample ignored alerts
...
-- ============================================================================
-- 5. USEFUL QUERIES FOR ALERTS SYSTEM
-- ============================================================================

-- Query to get all active low stock alerts with supplier information
/* 
SELECT 
  p.id,
  p.name,
...
-- Query to get ignored alerts with details
/*
SELECT 
  ia.id,
...
-- Query to get resolved alerts (purchase orders)
/*
SELECT 
  po.id,
  po.po_number,
...
/*
SELECT 
  COUNT(CASE WHEN current_stock <= low_stock_threshold AND alert_status = 'active' THEN 1 END) as pending_alerts,
  COUNT(CASE WHEN alert_status = 'ignored' THEN 1 END) as ignored_alerts,
  (SELECT COUNT(*) FROM purchase_orders WHERE DATE(created_at) = CURDATE()) as todays_purchase_orders,
...
-- Procedure to create purchase order
...
-- View for active alerts with all details
...
CREATE INDEX IF NOT EXISTS idx_products_low_stock ON products(current_stock, low_stock_threshold, alert_status);
CREATE INDEX IF NOT EXISTS idx_products_alert_time ON products(low_stock_alert_time);
CREATE INDEX IF NOT EXISTS idx_products_priority ON products(alert_priority);
CREATE INDEX IF NOT EXISTS idx_purchase_orders_status ON purchase_orders(status, created_at);
CREATE INDEX IF NOT EXISTS idx_ignored_alerts_product ON ignored_alerts(product_id, ignored_at);
