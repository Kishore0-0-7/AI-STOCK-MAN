...
class DashboardController {
  ...
  static async getActivity(req, res) {
    try {
      ...
      console.log(
        "Activity endpoint called with limit:",
        limit,
        "parsed as:",
        limitValue
      );
    ...
    try {
      ...
      if (salesTrends.length === 0 && inventoryTrends.length === 0) {
        ...
        for (let i = 0; i < parseInt(period); i++) {
          const monthIndex =
            (currentMonth - parseInt(period) + 1 + i + 12) % 12;
          monthlyData.push({
        ...
        allMonths.forEach((month) => {
          ...
          monthlyData.push({
            ...
            inventory: Math.round(
              parseFloat(inventoryData.total_quantity_sold || 0) * 5
            ), // Scale inventory
            orders: parseInt(salesData.orders || 0),
            total_quantity_sold: parseInt(
              inventoryData.total_quantity_sold || 0
            ),
            unique_products_sold: parseInt(
              inventoryData.unique_products_sold || 0
            ),
          });
        });
      }
  ...
  static async getSalesMetrics(req, res) {
    try {
      ...
      const [dailySales, totalRevenue, totalOrders, topProducts] =
        await Promise.all([
  ...
  static async getPurchaseMetrics(req, res) {
    try {
      ...
      const [pendingPOs, totalPurchaseValue, supplierPerformance, recentPOs] =
        await Promise.all([
      ...
      const metrics = {
        ...
        totalPurchaseValue: parseFloat(
          totalPurchaseValue[0][0]?.total_value || 0
        ),
  ...
  static async getWarehouseMetrics(req, res) {
    try {
      const [storageUtilization, locationSummary, capacityOverview] =
        await Promise.all([
      ...
      const metrics = {
        ...
        overallUtilization:
          capacityOverview[0][0]?.total_capacity > 0
            ? Math.round(
                (capacityOverview[0][0].total_occupied /
                  capacityOverview[0][0].total_capacity) *
                  100 *
                  100
              ) / 100
            : 0,
  ...
  static async getQualityMetrics(req, res) {
    try {
      ...
      const [qcSummary, defectsByType, inspectionTrends, holdItems] =
        await Promise.all([
          // QC Summary
          pool.execute(
            `
